...
/*add by andy for fp*/
#define CF_FP_DOWNLOAD 		2 
#define CF_FP_RELEASE  		1
...
{
	//printk("andy %s--> status : %d",__func__,is_release_cf_fp);
...
#define CF_NAV_INPUT_UP						0x2e8
#define CF_NAV_INPUT_DOWN					0x2e9
#define CF_NAV_INPUT_LEFT					0x2ea
#define CF_NAV_INPUT_RIGHT					0x2eb
#define CF_NAV_INPUT_CLICK					0x2ed
...
static int cdfinger_open(struct inode *inode, struct file *file)
{
	...
	is_release_cf_fp = CF_FP_DOWNLOAD;
	/*add by andy for fp test end*/
...
static irqreturn_t cdfinger_eint_handler(int irq, void *dev_id)
{
	...
	if((!screen_status) && (g_cdfingerfp_data->is_finger_down == true))
	{
...
/*add for fingerprint unlock screen by guoyicong*/
...
static int cdfinger_probe(struct platform_device *pdev)
{
    ...
    msm_drm_register_client(&cdfingerdev->notifier);
