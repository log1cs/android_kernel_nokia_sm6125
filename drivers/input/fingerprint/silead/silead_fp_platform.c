...
static int proc_show_ver(struct seq_file *file,void *v)
{
	...
	if(2 == cf_status)
	{
		seq_printf(file,"[Vendor]:cdfinger-FPS980SC\n");
	}

	if((1 == cf_status)||(0 == cf_status))
	{
		seq_printf(file,"[Vendor]:silead-GSL6150NX3\n");
	}
	return 0;
}
...
static nav_keymap_t keymap[] = {
    { NAV_KEY_UP,       SF_KEY_UP,         }, /* KEY_RESERVED, ignore this key */
    { NAV_KEY_DOWN,     SF_KEY_DOWN,       },
    { NAV_KEY_RIGHT,    SF_KEY_RIGHT,      },
    { NAV_KEY_LEFT,     SF_KEY_LEFT,       },
    { NAV_KEY_CLICK,    SF_KEY_HOMEPAGE,   },
};
                             ...
                             unsigned long event, void *data)
{
    ...
    if(evdata &&evdata->data &&event == MSM_DRM_EVENT_BLANK){
		//printk("andy enter silfp_fb_callback line:%d\n",__LINE__);
		blank = evdata->data;
...
static irqreturn_t silfp_irq_handler(int irq, void *dev_id)
{
	...
	if((!screen_status ) && (fp_dev->is_finger_down == true))
	{
		//printk("andy enter func:%s\n",__func__);		
		fp_dev->is_finger_down = false;
		schedule_work(&fp_dev->unlock_work);
	}
...
/*---add for fingerprint unlock screen by guoyicong start----------------*/
static void sileadfinger_fb_notification_unlock(struct work_struct *work)
{
	//SILEADFINGER_ERR("ssan sileadfinger_fb_notification_unlock  in \n");
	pr_debug("%s unblank\n", __func__);
	dsi_bridge_interface_enable(FP_UNLOCK_REJECTION_TIMEOUT);
}
/*---------add for fingerprint unlock screen by guoyicong end--------------*/
static int silfp_init(struct silfp_data *fp_dev)
{
    ...
    /* register screen on/off callback */
       ...
       msm_drm_register_client(&fp_dev->notif);
   // SIL_REGISTER_CLIENT(&fp_dev->notif);
	/*add for fingerprint unlock screen by guoyicong start*/
	INIT_WORK(&fp_dev->unlock_work,sileadfinger_fb_notification_unlock);
	fp_dev->is_finger_down = false;
	/*add for fingerprint unlock screen by guoyicong end*/
#endif /* CONFIG_HAS_EARLYSUSPEND */
...
static int silfp_probe(struct platform_device *spi)
{
	...
	if (NULL == proc_entry)
	{
		printk("silead_fp Couldn't create proc entry!");
		proc_entry = NULL;
	}
	else
	{
		printk("silead_fp Create proc entry success!");
	}
	/*add by andy for fp test end*/
