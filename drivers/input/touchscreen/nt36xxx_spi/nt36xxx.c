...
const uint16_t gesture_key_table[][2] = {
...
int nvt_gesture_switch(struct input_dev *dev, unsigned int type, unsigned int code, int value){
	...
	if ((type == EV_SYN) && (code == SYN_CONFIG) && 
		((value == WAKEUP_ON) || (value == WAKEUP_ON))) {
		lct_nvt_tp_gesture_callback((value == WAKEUP_ON)?true:false);
	}
...
{
//printk("andy enter fun:%s line:%d\n",__func__,__LINE__);
	if (unlikely(ts->is_resumed)) {
...
void nvt_ts_wakeup_gesture_report(uint8_t gesture_id, uint8_t *data)
{
	...
	for(index=0; gesture_key_table[index][0] != KEY_RESERVED; index++){
		if(gesture_key_table[index][1] == gesture_id){
			input_report_key(ts->input_dev, gesture_key_table[index][0], 1);
			input_sync(ts->input_dev);
			input_report_key(ts->input_dev, gesture_key_table[index][0], 0);
			input_sync(ts->input_dev);
...
#if (WAKEUP_GESTURE) && defined(CONFIG_PM)
...
static int8_t nvt_ts_check_chip_ver_trim(void)
{
	...
	for (retry = 5; retry > 0; retry--) {
		...
		NVT_LOG("buf: %02X %02X %02X %02X %02X %02X\n", buf[6], buf[5], buf[4], buf[3], buf[2], buf[1]);
		...
		for (list = 0; list < (sizeof(trim_id_table) / sizeof(struct nvt_ts_trim_id_table)); list++) {
			...
			for (i = 0; i < NVT_ID_BYTE_MAX; i++){
				if ((trim_id_table[list].mask[i]) && (buf[i + 1] != trim_id_table[list].id[i]))
					break;
            }

			if (i == NVT_ID_BYTE_MAX) {
				sprintf(ts->nova_touch_strId, "nt%x%x%x-%x", buf[6], buf[5], buf[4], (buf[1]==0xFF)?0:buf[1]);
				NVT_LOG("This is NVT touch IC:%s\n", ts->nova_touch_strId);
...
extern void touchscreen_update_state_regist(UpdateUsbState pfunc);
#endif
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
...
#if 0
	ret = nvt_ts_enable_regulator(SLEEP_POWER_ENABLE);//default disable regulator
	if (ret < 0) {
	...
	for (retry = 0; gesture_key_table[retry][0] != KEY_RESERVED; retry++) {
		input_set_capability(ts->input_dev, EV_KEY, gesture_key_table[retry][0]);
	}
#endif
...
#ifdef NVT_USB_CHECK_STATE
	ts->usb_plugin = false;
	touchscreen_update_state_regist(nvt_update_usb_state);
#endif
...
//err_enable_regulator:
	//nvt_ts_enable_regulator(!SLEEP_POWER_ENABLE);
err_get_regulator:
...
bool nt36xxx_reset_keep_high(void){
	if(!IS_ERR_OR_NULL(ts)){
		if(strstr(ts->nova_touch_strId, "nt36526") != NULL){
			return true;
		}
	}
...
EXPORT_SYMBOL(nt36xxx_reset_keep_high);
...
int lct_nvt_tp_gesture_callback(bool flag){
	...
	if(enable == ts->is_gesture_enable){
		return 0;
	}
	
	ts->is_gesture_enable = enable;
#if TOUCHPAD_REGULATOR && (!SLEEP_POWER_ENABLE)
	if(ts->is_gesture_mode){
		nvt_irq_enable(false);
		ts->is_gesture_mode = false;
	}
	nvt_ts_enable_regulator(ts->is_gesture_enable);
#endif
...
int32_t nvt_driver_init(void)
{
	...
	//---add spi driver---
	spi_register_driver(&nvt_spi_driver);
	if (IS_ERR_OR_NULL(ts)) {
