...
int nvt_tpd_usb_plugin(bool plugin)
{
	int ret = -1;
	int usb_state = EVENTBUFFER_STATUS_AC;

	usb_state = plugin?EVENTBUFFER_STATUS_AC:EVENTBUFFER_STATUS_OFF;

	NVT_LOG("%s", (usb_state==EVENTBUFFER_STATUS_AC)?"plugin":"plugout");
    ret = nvt_customizeCmd(usb_state);
    if (ret < 0){
        NVT_ERR("update usb state 0x%02X cmd failed.\n", usb_state);
    }

	return ret;
}
#endif //NVT_USB_CHECK_STATE

void nvt_update_usb_state(bool plugin){
#ifdef NVT_USB_CHECK_STATE
	ts->usb_plugin = plugin;

	if(!IS_ERR_OR_NULL(ts)){
		if(ts->is_resumed && ts->nvt_pid){
			nvt_tpd_usb_plugin(ts->usb_plugin);
		}
	}
#endif
}
