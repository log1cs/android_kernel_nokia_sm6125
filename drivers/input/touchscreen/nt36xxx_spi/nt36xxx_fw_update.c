...
int32_t nvt_update_firmware(char *firmware_name)
{
	...
	//Dont't Update firmware info and USB state while use selftest firmware
	if(strcmp(firmware_name, ts->mp_update_firmware_name)){
#ifdef CONFIG_TOUCHSCREEN_PROCFILE
	    {
	        ...
	        case TP_VENDOR_TIANMA:
	            strcpy(tp_info, "[Vendor]tianma,");
	            break;
	        case TP_VENDOR_EBBG:
	            strcpy(tp_info, "[Vendor]shenchao,");
	            break;
	        default:
	            strcpy(tp_info, "[Vendor]Unknow,");
	            break;
	        }
#endif
	        sprintf(tp_info+strlen(tp_info), "[FW]0x%02x,[IC]%s\n", ts->fw_ver, ts->nova_touch_strId);
	        NVT_ERR("%s", tp_info);
