...
static int ilitek_tddi_fw_iram_read(u8 *buf, u32 start, int len)
{
	...
	for (i = 0, addr = start; addr < end; i += limit, addr += limit) {
		...
		ili9881x_ts->fw_update_stat = (i * 100) / end;
		ILI_DBG("Reading iram data .... %d%c", ili9881x_ts->fw_update_stat, '%');
	}
	return 0;
}
...
out:
	...
	ili9881x_ts->fw_update_stat = (ret < 0) ? FW_UPDATE_FAIL : FW_UPDATE_PASS;
...
static int ilitek_tddi_fw_read_flash_data(u32 start, u32 end, u8 *data, int len)
{
	...
	for (i = start; i <= end; i++) {
		...
		index++;
		ili9881x_ts->fw_update_stat = (i * 100) / end;
		ILI_DBG("Reading flash data .... %d%c", ili9881x_ts->fw_update_stat, '%');
	}
...
static void ilitek_tddi_flash_protect(bool enable)
{
	...
	switch (ili9881x_ts->flash_mid) {
	case 0xEF:
		if (ili9881x_ts->flash_devid == 0x6012 || ili9881x_ts->flash_devid == 0x6011) {
			if (enable) {
		...
		break;
	case 0xC8:
		if (ili9881x_ts->flash_devid == 0x6012 || ili9881x_ts->flash_devid == 0x6013) {
			if (enable) {
...
static int ilitek_fw_check_ddi_chunk(int from_dma, u8 *pfw)
{
	int i, j, ret;
	u8 cmd[7] = {0x0};
	u32 start = DDI_RSV_BK_ST_ADDR, end = DDI_RSV_BK_END_ADDR;
	u32 len = end - start + 1;
	...
	ILI_INFO("Obtain block info data fw by sending cmd\n");
	for (i = 0; i < len; i += 0x400) {
		cmd[0] = 0x41;				   //cmd header
		cmd[1] = 0x0;				   //len L byte
		cmd[2] = 0x40;				   //len H byte
		cmd[3] = ((start + i) & 0x000000FF);       //addr L byte
		cmd[4] = ((start + i) & 0x0000FF00) >> 8;  //addr M byte
		cmd[5] = ((start + i) & 0x00FF0000) >> 16; //addr H byte
		cmd[6] = ili_calc_packet_checksum(cmd, sizeof(cmd) - 1);

		memset(fw_flash_tmp, 0x0, sizeof(fw_flash_tmp));

		ili9881x_ts->i2c_cmd_int = true;

		if (ili9881x_ts->wrapper(cmd, sizeof(cmd), fw_flash_tmp, sizeof(fw_flash_tmp), ON) < 0) {
			ILI_ERR("Read flash data from fw error\n");
			goto dma;
		}

		for (j = 0; j < 0x400; j++)
			comb_bk[j + i] = fw_flash_tmp[6 + j];
	}
	goto out;
...
dma:
	...
	if (ilitek_tddi_fw_read_flash_data(start, end, comb_bk, len) < 0) {
		ILI_ERR("Read flash data from dma error\n");
		ret = UPDATE_FAIL;
		goto out;
	}
...
out:
	/* Copy two block's data to pfw except for ddi data. */
	for (i = 0; i < len; i++) {
		if (((start + i) < fbi[DDI].start) || ((start + i) > fbi[DDI].end))
			pfw[start + i] = comb_bk[i];
	}
	...
	ili9881x_ts->i2c_cmd_int = false;
	return ret;
}

static u32 ili_fw_read_hw_crc(u32 start, u32 end)
{
	int retry = 500;
	u32 busy = 0;
	u32 write_len = end;
	...
	if (write_len > ili9881x_ts->chip->max_count) {
		ILI_ERR("The length (%x) written into firmware is greater than max count (%x)\n",
			write_len, ili9881x_ts->chip->max_count);
		return -1;
	}
...
static int ilitek_tddi_fw_check_ver(u8 *pfw)
{
	...
	/* Check FW version */
	ILI_INFO("New FW ver = 0x%x, Current FW ver = 0x%x\n", tfd.new_fw_cb, ili9881x_ts->chip->fw_ver);
	if (tfd.new_fw_cb != ili9881x_ts->chip->fw_ver) {
		ILI_INFO("FW version is different, do upgrade\n");
		return UPDATE_FAIL;
	}
...
static int ilitek_tddi_fw_flash_erase(void)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		for (addr = fbi[i].start; addr <= fbi[i].end; addr += ili9881x_ts->flash_sector) {
			ilitek_tddi_flash_write_enable();
...
static int ilitek_tddi_fw_flash_program(u8 *pfw)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		for (addr = fbi[i].start; addr < fbi[i].end; addr += ili9881x_ts->program_page) {
			buf[0] = 0x25;
			buf[3] = 0x04;
			buf[2] = 0x10;
			buf[1] = 0x08;

			for (k = 0; k < ili9881x_ts->program_page; k++) {
			...
			if (ili9881x_ts->wrapper(buf, ili9881x_ts->program_page + 4, NULL, 0, OFF) < 0) {
			...
			ili9881x_ts->fw_update_stat = (addr * 101) / tfd.end_addr;
			if (ili9881x_ts->fw_update_stat > FW_UPDATING)
...
static int ilitek_fw_flash_upgrade(u8 *pfw)
{
	...
	ilitek_fw_check_ddi_chunk(ret, pfw);
...
static int ilitek_tdd_fw_hex_open(u8 op, u8 *pfw)
{
	...
	case FILP_OPEN:
		...
		ili9881x_ts->tp_fw.size = 0;
		ili9881x_ts->tp_fw.data = vmalloc(fsize);
		if (ili9881x_ts->tp_fw.data == NULL) {
			ILI_ERR("Failed to allocate tp_fw by vmalloc, try again\n");
			ili9881x_ts->tp_fw.data = vmalloc(fsize);
			if (ili9881x_ts->tp_fw.data == NULL) {
				ILI_ERR("Failed to allocate tp_fw after retry\n");
...
static void ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	ili9881x_ts->trans_xy = (ili9881x_ts->chip->core_ver >= CORE_VER_1430) ? ili9881x_ts->fw_info[0] : OFF;
...
void ili_fw_read_flash_info(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(flash_t); i++) {
		if (flash_mid == flash_t[i].mid && flash_id == flash_t[i].dev_id) {
			ili9881x_ts->flash_mid = flash_t[i].mid;
			ili9881x_ts->flash_devid = flash_t[i].dev_id;
			ili9881x_ts->program_page = flash_t[i].program_page;
			ili9881x_ts->flash_sector = flash_t[i].sector;
			break;
		}
	}
	...
	if (i >= ARRAY_SIZE(flash_t)) {
		...
		ili9881x_ts->flash_mid = flash_t[0].mid;
		ili9881x_ts->flash_devid = flash_t[0].dev_id;
		ili9881x_ts->program_page = flash_t[0].program_page;
		ili9881x_ts->flash_sector = flash_t[0].sector;
	}

	ILI_INFO("Flash MID = %x, Flash DEV_ID = %x\n", ili9881x_ts->flash_mid, ili9881x_ts->flash_devid);
	ILI_INFO("Flash program page = %d\n", ili9881x_ts->program_page);
	ILI_INFO("Flash sector = %d\n", ili9881x_ts->flash_sector);
