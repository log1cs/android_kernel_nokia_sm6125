...
static int ilitek_debug_node_buff_control(bool open)
{
	...
	ili9881x_ts->dnp = open;

	ILI_INFO("Debug buf ctrl = %s\n", ili9881x_ts->dnp ? "Enabled" : "Disabled");

	if (open) {
		ili9881x_ts->dbf = 0;
		ili9881x_ts->odi = 0;
		ipio_kfree((void **)&ili9881x_ts->dbl);
		ili9881x_ts->dbl = kzalloc(TR_BUF_LIST_SIZE * sizeof(*ili9881x_ts->dbl), GFP_KERNEL);
		if (ERR_ALLOC_MEM(ili9881x_ts->dbl)) {
			ILI_ERR("Failed to allocate ili9881x_ts->dbl mem, %ld\n", PTR_ERR(ili9881x_ts->dbl));
			ret = -ENOMEM;
			goto out;
		}

		for (i = 0; i < TR_BUF_LIST_SIZE; i++) {
			ili9881x_ts->dbl[i].mark = false;
			ipio_kfree((void **)&ili9881x_ts->dbl[i].data);
			ili9881x_ts->dbl[i].data = kzalloc(TR_BUF_SIZE * sizeof(unsigned char), GFP_KERNEL);
			if (ERR_ALLOC_MEM(ili9881x_ts->dbl[i].data)) {
				ILI_ERR("Failed to allocate dbl[%d] mem, %ld\n", i, PTR_ERR(ili9881x_ts->dbl[i].data));
				ret = -ENOMEM;
				goto out;
			}
		}
...
static int debug_mode_get_data(struct file_buffer *file, u8 type, u32 frame_count)
{
	...
	ili9881x_ts->dbf = 0;
	row = ili9881x_ts->ych_num;
	col = ili9881x_ts->xch_num;
	...
	while (write_index < frame_count) {
		file->wlen = 0;
		ILI_INFO("frame = %d,index = %d,count = %d\n", write_index,ili9881x_ts->odi, ili9881x_ts->dbf);
		if (!wait_event_interruptible_timeout(ili9881x_ts->inq, ili9881x_ts->dbl[ili9881x_ts->odi].mark, msecs_to_jiffies(3000))) {
		...
		ptr = &ili9881x_ts->dbl[ili9881x_ts->odi].data[35];
		...
		ili9881x_ts->dbl[ili9881x_ts->odi].mark = false;
		ili9881x_ts->odi = ((ili9881x_ts->odi + 1) % TR_BUF_LIST_SIZE);
	}
out:
	ilitek_debug_node_buff_control(DISABLE);
	return ret;
...
static ssize_t ilitek_proc_get_delta_data_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	cmd[0] = 0xB7;
	cmd[1] = 0x1; //get delta
	ret = ili9881x_ts->wrapper(cmd, sizeof(cmd), NULL, 0, ON);
	if (ret < 0) {
	...
	ret = ili9881x_ts->wrapper(NULL, 0, data, read_length, ON);
	if (ret < 0) {
...
static ssize_t ilitek_proc_rw_tp_reg_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	bool esd_en = ili9881x_ts->wq_esd_ctrl, bat_en = ili9881x_ts->wq_bat_ctrl;
...
static ssize_t ilitek_proc_debug_message_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	mutex_lock(&ili9881x_ts->debug_read_mutex);
	ILI_DBG("f_count= %d, index = %d, mark = %d\n", ili9881x_ts->dbf, ili9881x_ts->odi, ili9881x_ts->dbl[ili9881x_ts->odi].mark);
	if (!wait_event_interruptible_timeout(ili9881x_ts->inq, ili9881x_ts->dbl[ili9881x_ts->odi].mark, msecs_to_jiffies(3000))) {
		ILI_ERR("WARNING ! there's no data received.\n");
		mutex_unlock(&ili9881x_ts->debug_read_mutex);
		*pos = send_data_len;
	...
	if (ERR_ALLOC_MEM(tmpbuf)) {
		...
		ret = copy_to_user(buff, tmpbufback, send_data_len); /*ili9881x_ts->dbl[0] */
		goto out;
	}

	if (ili9881x_ts->dbl[ili9881x_ts->odi].mark) {
		if (ili9881x_ts->dbl[ili9881x_ts->odi].data[0] == P5_X_DEMO_PACKET_ID) {
			need_read_data_len = 43;
		} else if (ili9881x_ts->dbl[ili9881x_ts->odi].data[0] == P5_X_I2CUART_PACKET_ID) {
			type = ili9881x_ts->dbl[ili9881x_ts->odi].data[3] & 0x0F;

			data_count = ili9881x_ts->dbl[ili9881x_ts->odi].data[1] * ili9881x_ts->dbl[ili9881x_ts->odi].data[2];
		...
		} else if (ili9881x_ts->dbl[ili9881x_ts->odi].data[0] == P5_X_DEBUG_PACKET_ID) {
			send_data_len = 0;	/* ili9881x_ts->dbl[0][1] - 2; */
			need_read_data_len = TR_BUF_SIZE - 8;
		}

		for (i = 0; i < need_read_data_len; i++) {
			send_data_len += snprintf(tmpbuf + send_data_len, sizeof(tmpbufback), "%02X", ili9881x_ts->dbl[ili9881x_ts->odi].data[i]);
...
static ssize_t ilitek_proc_get_debug_mode_data_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	ret = debug_mode_get_data(&csv, P5_X_FW_RAW_DATA_MODE, ili9881x_ts->raw_count);
	if (ret < 0)
		goto out;
	...
	ret = debug_mode_get_data(&csv, P5_X_FW_DELTA_DATA_MODE, ili9881x_ts->delta_count);
	if (ret < 0)
		goto out;

	/* change to demo mode */
	if (ili_set_tp_data_len(DATA_FORMAT_DEMO) < 0)
		ILI_ERR("Failed to set tp data length\n");
...
static ssize_t ilitek_proc_get_debug_mode_data_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	ili9881x_ts->raw_count = ((temp[0] << 8) | temp[1]);
	ili9881x_ts->delta_count = ((temp[2] << 8) | temp[3]);
	ili9881x_ts->bg_count = ((temp[4] << 8) | temp[5]);

	ILI_INFO("Raw_count = %d, Delta_count = %d, BG_count = %d\n", ili9881x_ts->raw_count, ili9881x_ts->delta_count, ili9881x_ts->bg_count);
	return size;
}
...
static ssize_t ilitek_node_fw_upgrade_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	ili9881x_ts->node_update = true;

	ret = ili_fw_upgrade_handler(NULL);

	ili9881x_ts->node_update = false;
...
int ili_get_tp_recore_data(void)
{
	...
	if (ili9881x_ts->wrapper(NULL, 0, buf, sizeof(buf), OFF) < 0) {
	...
	if (ili9881x_ts->wrapper(buf, 4, NULL, 0, true)) {
		ILI_ERR("Failed to write iram data\n");
		return -ENODEV;
	}

	if (ili9881x_ts->wrapper(NULL, 0, (u8 *)raw, len, true)) {
	...
	for (i = 0; i < fcnt; i ++) {
		...
		ili_dump_data(raw_ptr, 16, frame_len, ili9881x_ts->xch_num, "recore_data");
		index--;
		if(index < 0)
			index = fcnt - 1;
	}
...
void ili_gesture_fail_reason(bool enable)
{
	...
	cmd[4] = 0xFF;
	cmd[5] = 0xFF;
	if ((ili9881x_ts->wrapper(cmd, 6, NULL, 0, ON)) < 0)
	...
	if ((ili9881x_ts->wrapper(cmd, 24, NULL, 0, ON)) < 0)
	...
	cmd[2] = 0x11;
	cmd[3] = 0x01;
	if ((ili9881x_ts->wrapper(cmd, 4, NULL, 0, ON)) < 0)
...
static ssize_t ilitek_node_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	if (strncmp(cmd, "hwreset", strlen(cmd)) == 0) {
		...
		ili_set_tp_data_len(DATA_FORMAT_DEMO_DEBUG_INFO);
	} else if (strcmp(cmd, "gesturedemoen") == 0) {
		if (data[1] == 0)
			ili9881x_ts->gesture_demo_ctrl = DISABLE;
		else
			ili9881x_ts->gesture_demo_ctrl = ENABLE;
		ili_set_tp_data_len(DATA_FORMAT_GESTURE_DEMO);
		...
		ili9881x_ts->info_from_hex = data[1];
		...
		ili9881x_ts->wq_ctrl = !ili9881x_ts->wq_ctrl;
		...
		ili9881x_ts->gesture = !ili9881x_ts->gesture;
		ILI_INFO("gesture = %d\n", ili9881x_ts->gesture);
		...
		ILI_INFO("ice mode flag = %d\n", atomic_read(&ili9881x_ts->ice_stat));
	} else if (strncmp(cmd, "gesturenormal", strlen(cmd)) == 0) {
		ili9881x_ts->gesture_mode = DATA_FORMAT_GESTURE_NORMAL;
		...
		ili9881x_ts->netlink = !ili9881x_ts->netlink;
		...
		ili9881x_ts->wrapper(temp, w_len, NULL, 0, ON);
	} else if (strncmp(cmd, "ior", strlen(cmd)) == 0) {
		int r_len = 0;
		r_len = data[1];
		ILI_INFO("r_len = %d\n", r_len);
		ili9881x_ts->wrapper(NULL, 0, temp, r_len, ON);
		for (i = 0; i < r_len; i++)
			ILI_INFO("read[%d] = %x\n", i, temp[i]);
		...
		ret = ili9881x_ts->wrapper(temp, w_len, NULL, 0, ON);
		...
		ret = ili9881x_ts->wrapper(temp, w_len, temp, r_len, ON);
		for (i = 0; i < r_len; i++)
			ILI_INFO("read[%d] = %x\n", i, temp[i]);
		...
		ili9881x_ts->spi_write_then_read(ili9881x_ts->spi, temp, wlen, NULL, 0);
		...
		temp[0] = 0x83;
		ili9881x_ts->spi_write_then_read(ili9881x_ts->spi, temp, 1, rbuf, rlen);
		for (i = 0; i < rlen; i++)
			ILI_INFO("read[%d] = %x\n", i, rbuf[i]);
		kfree(rbuf);
		...
		ili9881x_ts->spi_write_then_read(ili9881x_ts->spi, temp, wlen, rbuf, rlen);
		if (rlen != 0) {
			for (i = 0; i < rlen; i++)
				ILI_INFO("read[%d] = %x\n", i, rbuf[i]);
		}
...
static long ilitek_node_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	u32 id_to_user[64] = {0};
	bool esd_en = ili9881x_ts->wq_esd_ctrl, bat_en = ili9881x_ts->wq_bat_ctrl;
	u8 *wrap_rbuf = NULL, wrap_wlen = 0, wrap_rlen = 0, wrap_int = 0, spi_irq;
	...
	case ILITEK_IOCTL_I2C_WRITE_DATA:
		...
		ret = ili9881x_ts->wrapper(szBuf, i2c_rw_length, NULL, 0, OFF);
		if (ret < 0)
	...
	case ILITEK_IOCTL_I2C_READ_DATA:
		...
		ret = ili9881x_ts->wrapper(NULL, 0, szBuf, i2c_rw_length, OFF);
		if (ret < 0) {
	...
	case ILITEK_IOCTL_TP_FUNC_MODE:
		...
		ret = ili9881x_ts->wrapper(szBuf, 3, NULL, 0, ON);
		break;
	case ILITEK_IOCTL_TP_FW_VER:
		ILI_DBG("ioctl: get fw version\n");
		szBuf[3] = ili9881x_ts->chip->fw_ver & 0xFF;
		szBuf[2] = (ili9881x_ts->chip->fw_ver >> 8) & 0xFF;
		szBuf[1] = (ili9881x_ts->chip->fw_ver >> 16) & 0xFF;
		szBuf[0] = ili9881x_ts->chip->fw_ver >> 24;
	...
	case ILITEK_IOCTL_TP_PL_VER:
		...
		szBuf[2] = ili9881x_ts->protocol->ver & 0xFF;
		szBuf[1] = (ili9881x_ts->protocol->ver >> 8) & 0xFF;
		szBuf[0] = ili9881x_ts->protocol->ver >> 16;
	...
	case ILITEK_IOCTL_TP_CORE_VER:
		...
		szBuf[3] = ili9881x_ts->chip->core_ver & 0xFF;
		szBuf[2] = (ili9881x_ts->chip->core_ver >> 8) & 0xFF;
		szBuf[1] = (ili9881x_ts->chip->core_ver >> 16) & 0xFF;
		szBuf[0] = ili9881x_ts->chip->core_ver >> 24;
	...
	case ILITEK_IOCTL_TP_CHIP_ID:
		...
		id_to_user[0] = ili9881x_ts->chip->pid;
		id_to_user[1] = ili9881x_ts->chip->otp_id;
		id_to_user[2] = ili9881x_ts->chip->ana_id;
	...
	case ILITEK_IOCTL_TP_NETLINK_STATUS:
		...
		if (copy_to_user((int *)arg, &ili9881x_ts->netlink, sizeof(int))) {
			ILI_ERR("Failed to copy driver ver to user space\n");
			ret = -ENOTTY;
		}
		break;
	...
	case ILITEK_IOCTL_TP_MODE_STATUS:
		ILI_DBG("ioctl: current firmware mode = %d", ili9881x_ts->actual_tp_mode);
		if (copy_to_user((int *)arg, &ili9881x_ts->actual_tp_mode, sizeof(int))) {
			ILI_ERR("Failed to copy driver ver to user space\n");
			ret = -ENOTTY;
		}
		break;
	...
	case ILITEK_IOCTL_TP_INTERFACE_TYPE:
		if_to_user = ili9881x_ts->hwif->bus_type;
	...
	case ILITEK_IOCTL_TP_FW_UART_CTRL:
		...
		if_to_user = ili9881x_ts->fw_uart_en;
	...
	case ILITEK_IOCTL_TP_PANEL_INFO:
		...
		id_to_user[0] = ili9881x_ts->panel_wid;
		id_to_user[1] = ili9881x_ts->panel_hei;
	...
	case ILITEK_IOCTL_TP_INFO:
		...
		id_to_user[0] = ili9881x_ts->min_x;
		id_to_user[1] = ili9881x_ts->min_y;
		id_to_user[2] = ili9881x_ts->max_x;
		id_to_user[3] = ili9881x_ts->max_y;
		id_to_user[4] = ili9881x_ts->xch_num;
		id_to_user[5] = ili9881x_ts->ych_num;
		id_to_user[6] = ili9881x_ts->stx;
		id_to_user[7] = ili9881x_ts->srx;
	...
	case ILITEK_IOCTL_WRAPPER_RW:
		...
		ILI_DBG("i2c_irq = %d, spi_irq = %d\n", ili9881x_ts->i2c_cmd_int, spi_irq);

		ili9881x_ts->wrapper(szBuf + 5, wrap_wlen, wrap_rbuf, wrap_rlen, spi_irq);
