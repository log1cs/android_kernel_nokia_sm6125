...
static s32 *frame_buf = NULL;
static s32 *key_buf = NULL;
static s32 *frame1_cbk700 = NULL, *frame1_cbk250 = NULL, *frame1_cbk200 = NULL;
static s32 *cap_dac = NULL, *cap_raw = NULL;
...
static int parser_get_ini_key_value(char *section, char *key, char *value)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		...
		if (ipio_strcmp(key, ini_info[i].key_name) == 0) {
			ipio_memcpy(value, ini_info[i].pKeyValue, ini_info[i].key_val_len, PARSER_MAX_KEY_VALUE_LEN);
			ILI_DBG("(key: %s, value:%s) => (ini key: %s, val: %s)\n",
					key,
					value,
					ini_info[i].key_name,
					ini_info[i].pKeyValue);
			ret = 0;
			break;
		}
	}
...
static void parser_ini_nodetype(s32 *type_ptr, char *desp, int frame_len)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		...
		for (j = 0, index1 = 0; j <= ini_info[i].key_val_len; j++) {
			if (ini_info[i].pKeyValue[j] == ';' || j == ini_info[i].key_val_len) {
				if (record != '.') {
					memset(str, 0, sizeof(str));
					ipio_memcpy(str, &ini_info[i].pKeyValue[index1], (j - index1), sizeof(str));
					temp = ili_katoi(str);
...
static void parser_ini_benchmark(s32 *max_ptr, s32 *min_ptr, int8_t type, char *desp, int frame_len)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		...
		record = ',';
		for (j = 0, index1 = 0; j <= ini_info[i].key_val_len; j++) {
			if (ini_info[i].pKeyValue[j] == ',' || ini_info[i].pKeyValue[j] == ';' ||
				ini_info[i].pKeyValue[j] == '.' || j == ini_info[i].key_val_len) {
...
static int ilitek_tddi_mp_ini_parser(const char *path)
{
	...
	ILI_INFO("ini file path = %s\n", ili9881x_ts->md_ini_rq_path);

	if (request_firmware(&fw, ili9881x_ts->md_ini_rq_path, ili9881x_ts->dev) < 0) {
		ILI_ERR("Failed to open ini file at\n");
		return -ENOENT;
	}
	...
	for (i = 0; i < PARSER_MAX_KEY_NUM; i++) {
		...
		memset(ini_info[i].pKeyValue, 0, PARSER_MAX_KEY_VALUE_LEN);
...
static s32 open_c_formula(int dac, int raw, int tvch, int gain)
{
	...
	if (id == ILI9881_CHIP) {
		//ret = ((dac * 414 * 39 / 2) + ((raw - 8192) * 36 * (7 * 100 - 22) * 10 / 16384)) / tvch / 100 / gain; //9881H
		ret = (dac * 3206 * 39 / 2 + (raw - 8192) * 9 * 7 * 125 * 5 / 256) / tvch / 1000 / gain;              //9881X
		return ret;
	}
...
static int allnode_key_cdc_data(int index)
{
	...
	ret = ili9881x_ts->wrapper(cmd, sizeof(cmd), ori, len, ON);
	if (ret < 0) {
...
// 		if (ili9881x_ts->wrapper(cmd, 2, NULL, 0, OFF) < 0) {
...
// 		if (ili9881x_ts->wrapper(&cmd[1], 1, NULL, 0, OFF) < 0) {
...
// 	}
	...
	if (ili9881x_ts->wrapper(cmd, core_mp.cdc_len, ori, len, ON) < 0) {
...
static int check_int_level(bool high)
{
...
static int pin_test(int index)
{
	...
	if (tItems[index].test_int_pin == ENABLE) {
		...
		cmd[1] = 0x1;
		ret = ili9881x_ts->wrapper(cmd, 2, NULL, 0, OFF);
		if (ret < 0) {
	...
	if (tItems[index].int_pulse_test == ENABLE) {
		...
		ret = ili9881x_ts->wrapper(cmd, 3, NULL, 0, OFF);
		if (ret < 0) {
			ILI_ERR("Write command failed\n");
			ret = -EMP_CMD;
			goto out;
...
static void ilitek_tddi_mp_init_item(void)
{
	...
	core_mp.chip_pid = ili9881x_ts->chip->pid;
	core_mp.chip_id = ili9881x_ts->chip->id;
	core_mp.chip_type = ili9881x_ts->chip->type;
	core_mp.chip_ver = ili9881x_ts->chip->ver;
	core_mp.fw_ver = ili9881x_ts->chip->fw_ver;
	core_mp.protocol_ver = ili9881x_ts->protocol->ver;
	core_mp.core_ver = ili9881x_ts->chip->core_ver;
	core_mp.cdc_len = ili9881x_ts->protocol->cdc_len;
	core_mp.no_bk_shift = ili9881x_ts->chip->no_bk_shift;
	core_mp.xch_len = ili9881x_ts->xch_num;
	core_mp.ych_len = ili9881x_ts->ych_num;
...
int ili_mp_test_main(char *apk, bool lcm_on)
{
	...
	if (ili9881x_ts->xch_num <= 0 || ili9881x_ts->ych_num <= 0) {
		ILI_ERR("Invalid frame length (%d, %d)\n", ili9881x_ts->xch_num, ili9881x_ts->ych_num);
		ret = -EMP_INVAL;
		goto out;
	}
	...
	ret = ilitek_tddi_mp_ini_parser(ili9881x_ts->md_ini_path);
	if (ret < 0) {
